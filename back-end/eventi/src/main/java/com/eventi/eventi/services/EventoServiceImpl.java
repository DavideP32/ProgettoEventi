...
public class EventoServiceImpl implements EventoService {
 ...
 /* -------------------------------------------------------------------------- */
    ...
    public Evento aggiornaPostiEvento(Evento trovato, int posti) {
        // Recupera l'evento dal database per evitare problemi di persistenza
        Evento eventoPersistito = eventoRepository.findById(trovato.getId())
                .orElseThrow(() -> new IllegalArgumentException("Evento non trovato."));

        // Verifica che ci siano posti sufficienti
        if (eventoPersistito.getPosti() < posti) {
            throw new IllegalArgumentException("Posti insufficienti per l'evento.");
        }

        // Aggiorna i posti disponibili
        eventoPersistito.setPosti(eventoPersistito.getPosti() - posti);
